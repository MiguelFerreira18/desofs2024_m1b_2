<script lang="ts">
	...
	import type { DeliveryDTOPatch } from '$lib/Types/types';
	import { sendRequest } from '$lib/scripts';
	import { EstadoEncomenda } from '$lib/Enum/enums';
    export let data: PageData;

    let meals = data.encomendas.mealsPerWeek;
	let people = data.encomendas.numberOfPeople;
	let price = data.encomendas.price;
	let pacoteId = data.encomendas.pacote.pacoteId;
    
    async function handleSubmit() {
		const encData: DeliveryDTOPatch = {
			encomendaId: data.encomendas.encomendaId,
			mealsPerWeek: meals,
...
<div class="bg-gray-100 px-20 pt-10 min-h-screen">
	<h1 class="text-5xl font-bold mb-8 text-center">Edit Delivery</h1>
